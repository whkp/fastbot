...
The WebUI ships **inside the published wheel** with no separate frontend build. Fastbot's source
distribution includes the same bundled workbench; no separate runtime frontend service is needed.
It supports persistent topics, temporary chats, visible agent activity, workspace controls, Apps,
Skills, Automations, and settings.
